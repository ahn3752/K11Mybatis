...
public class MybatisController {
	...
	/*
	 servlet-context.xml에서 생성된 빈을 자동으로 주입받아
	 Mybatis를 사용할 준비를 한다.
	...
	{	
		...
		if(req.getParameter("searchTxt")!=null) {
			...
			//검색어는 스페이스로 구분되므로 split()을 통해 문자열 배열로 만들어준다.
			String[] sTxArray = req.getParameter("searchTxt").split(" ");
			//배열의 크기만큼 반복하면서 문자열을 컬렉션에 저장한다.
		...
		//게시물의 갯수를 카운트
		/*
		 서비스 객체 역할을 하는 인터페이스에 정의된 추상메소드를 호출하면
		 최종적으로 Mapper의 동일한 id속성값을 가진 엘리먼트의 쿼리문이 실행되어
		 결과를 반환받게 된다. 
	...
	public String write(Model model, HttpSession session, HttpServletRequest req) {
		...
		if(session.getAttribute("siteUserInfo")==null) {
			...
			//로그인 후 즉시 쓰기페이지로 로케이션하기 위해 뷰 경로를 파라미터로 넘겨준다.
	...
	{	
		...
		if(backUrl==null || backUrl.equals("")) {
			//로그인 버튼으로 로그인 한 경우에는 로그인 페이지로 이동한다.
			mv.setViewName("07Mybatis/login");
		}
		else {
			//특정페이지로 진입후 로그인 페이지로 이동했다면 기존 페이지로 이동한다.
	...
	public String modifyAction(HttpSession session, MyBoardDTO myBoardDTO) {
		...
		//수정폼에서 전송한 모든 폼값을 한꺼번에 저장한 커맨드 객체를 사용한다.
